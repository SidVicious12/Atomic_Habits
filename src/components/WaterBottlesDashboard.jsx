...
const WaterBottlesDashboard = () => {
  ...
  const chartData = useMemo(() => {
    ...
    while (current.compare(dateRange.end) <= 0) {
      ...
      const value = waterBottlesData[year]?.[monthName] || 0;
      data.push({
  ...
  return (
    <div className="p-6 rounded-2xl shadow-lg bg-white w-full min-h-[400px] transition-shadow hover:shadow-xl flex flex-col">
      <div className="flex justify-between items-start flex-wrap gap-4 border-b pb-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Water Bottles Analysis</h2>
          <p className="text-sm text-gray-500 mt-1">Date range analysis of water consumption.</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <Tooltip
              ...
              cursor={{ fill: 'rgba(54, 162, 235, 0.1)' }}
            />
            <Legend wrapperStyle={{ paddingTop: '20px' }} />
            <Bar dataKey="value" fill="#36a2eb" name="Water Bottles" />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
