...
import {
  ...
  LinearXAxisTickLabel, LinearYAxis, LinearYAxisTickSeries, LinearYAxisTickLabel, GridlineSeries,
...
export const AreaChartXs: React.FC<AreaChartXsProps> = ({
  ...
  return (
    <AreaChart id={id} data={data} width={width} height={height}
      series={<AreaSeries colorScheme={[colorScheme]} area={<Area gradient={<Gradient stops={[<GradientStop key="0" stopOpacity={0} offset="0%" />, <GradientStop key="1" offset="100%" stopOpacity={themeColors.gradientStop2Opacity} />]} />} />} />}
      xAxis={
        <LinearXAxis
          type="time"
          axisLine={null}
          tickSeries={
            showXAxisTicks ? (
              <LinearXAxisTickSeries
                tickSize={10}
                label={<LinearXAxisTickLabel format={xAxisFormat} fill={themeColors.axisTickColor} />}
              />
            ) : (
              <LinearXAxisTickSeries line={null} label={null} tickSize={0} />
            )
          }
        />
      }
          ...
          tickSeries={
            showYAxisTicks ? (
              <LinearYAxisTickSeries
                tickSize={10}
                label={<LinearYAxisTickLabel fill={themeColors.axisTickColor} />}
              />
            ) : (
              <LinearYAxisTickSeries line={null} label={null} tickSize={0} />
            )
          }
        />
      }
