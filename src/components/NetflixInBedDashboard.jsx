...
const NetflixInBedDashboard = () => {
  ...
  const chartData = useMemo(() => {
    ...
    while (current.compare(dateRange.end) <= 0) {
      ...
      const value = netflixData[year]?.[monthName] || 0;
      data.push({
  ...
  return (
    <div className="p-6 rounded-2xl shadow-lg bg-white w-full min-h-[400px] transition-shadow hover:shadow-xl flex flex-col">
      <div className="flex justify-between items-start flex-wrap gap-4 border-b pb-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Netflix in Bed Analysis</h2>
          <p className="text-sm text-gray-500 mt-1">Date range analysis of viewing habit.</p>
        </div>
      ...
      <div className="flex-grow mt-6">
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <Bar dataKey="value" fill="#8b5cf6" name="Watched" isAnimationActive={true} animationDuration={500} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
